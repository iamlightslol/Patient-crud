...
class SiteHelpers
{
							...
							{			
								$f .= '					
								  <div class="form-group '.$hidethis.' '.$inhide.'" '.$show .'>
									...
									<?php echo SiteHelpers::activeLang(\''.$form['label'].'\', (isset($fields[\''.$form['field'].'\'][\'language\'])? $fields[\''.$form['field'].'\'][\'language\'] : array()))  ;?>				
									'.$required.'</label>
									<div class="col-md-8">
			...
			if($grid['detail'] =='1')  
			{
				...
				{	
					...
					if(isset($c[2]))
					{
						...
						$val 	= "<?php echo ".$c[0]."::".$c[1]."(".substr($ar,0,strlen($ar)-1).") ;?>";
				...
				$f .= "
					<tr>
						...
						<?php echo SiteHelpers::activeLang('".$grid['label']."', (isset(\$fields['".$grid['field']."']['language'])? \$fields['".$grid['field']."']['language'] : array())) ;?>
						</td>
						<td>".$val." </td>
	...
	public static  function transForm( $field, $forms = array(),$bulk=false , $value ='')
	{
		...
		{
			default;
				$form ="<input  type='text' name='".$field."{$bulk}' class='form-control input-sm' $mandatory value='{$value}' placeholder='{$label}' />";
				break;
			...
			case 'text_date';
				$form = "<input  type='text' name='$field{$bulk}' class='date form-control input-sm' $mandatory value='{$value}' placeholder='{$label}'/> ";
				break;
